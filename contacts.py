...
def ask_until_option_expected(options):
    ...
    while not selected_action.isdigit() or (selected_action.isdigit() and int(selected_action) not in options ):
        selected_action = input("¿Qué opcion deseas? ")
...
def remove_contact(contacts):
    print("\n\nRemover Contacto\n")
    remove = input("Que contacto quieres eliminar: ")
    for contact in contacts:
        if contact["name"].find(remove) == 0:
            del contact["name" ]
...
def find_contact(contacts):
        ...
        print("He econtrados los siguientes contactos")
        contact_indexes = []
        contact_counter = 0
        ...
        if len(contact_indexes) > 1:
            ...
            print("No se ha encotrado ninguno")
            return
